...
export default function CabangPembelianTable({ data }: Props) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Pembelian per Cabang</CardTitle>
        <CardDescription>Breakdown pembelian setiap cabang</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {data.map((cabang, index) => (
              <TableRow key={index}>
                ...
                <TableCell className="text-right">{formatCurrency(cabang.total_pembelian)}</TableCell>
